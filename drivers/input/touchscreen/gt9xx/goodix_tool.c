...
ssize_t goodix_tool_write(struct file *filp, const char __user *buff, size_t len, loff_t *off)
{
    ...
    ret = raw_copy_from_user(&cmd_head, buff, CMD_HEAD_LENGTH);
    if(ret)
    {
        GTP_ERROR("raw_copy_from_user failed.");
        return -EPERM;
    }
    ...
    if (1 == cmd_head.wr)
    {
        ret = raw_copy_from_user(&cmd_head.data[GTP_ADDR_LENGTH], &buff[CMD_HEAD_LENGTH], cmd_head.data_len);
        if(ret)
        {
            GTP_ERROR("raw_copy_from_user failed.");
            return -EPERM;
    ...
    {
        ret = raw_copy_from_user(&cmd_head.data[0], &buff[CMD_HEAD_LENGTH], cmd_head.data_len);
        if(ret)
        {
            GTP_ERROR("raw_copy_from_user failed.");
            return -EPERM;
    ...
    else if(17 == cmd_head.wr)
    {
        ...
        if(ret)
        {
            GTP_DEBUG("raw_copy_from_user failed.");
            return -EPERM;
        }
