...
void gtp_int_sync(s32 ms, s32 gpio_irq_port);
...
extern s32 gup_clk_calibration(s32 gpio_irq_port);
...
static void gtp_reverse_xy (struct goodix_ts_data* ts, s32 *x, s32 *y) {
	*x = ts->abs_x_max - *x;
	*y = ts->abs_y_max - *y;
}
...
void gtp_int_sync(s32 ms, s32 gpio_irq_port)
{
    GTP_GPIO_OUTPUT(gpio_irq_port, 0);
    msleep(ms);
    GTP_GPIO_AS_INT(gpio_irq_port);
}
...
// #if GTP_COMPATIBLE_MODE
    struct goodix_ts_data *ts = i2c_get_clientdata(client);
// #endif    
    s32 gpio_irq_port = ts->int_gpio;
    s32 gpio_reset_port = ts->reset_gpio;

    GTP_DEBUG_FUNC();
    GTP_INFO("Guitar reset");
    GTP_GPIO_OUTPUT(gpio_reset_port, 0);   // begin select I2C slave addr
    msleep(ms);                         // T2: > 10ms
    // HIGH: 0x28/0x29, LOW: 0xBA/0xBB
    GTP_GPIO_OUTPUT(gpio_irq_port, client->addr == 0x14);

    msleep(2);                          // T3: > 100us
    GTP_GPIO_OUTPUT(gpio_reset_port, 1);
    
    msleep(6);                          // T4: > 5ms

    GTP_GPIO_AS_INPUT(gpio_reset_port);    // end select I2C slave addr
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data * ts)
{
    ...
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
        ...
        GTP_GPIO_OUTPUT(gpio_irq_port, 1);
        msleep(5);
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
    ...
    GTP_DEBUG_FUNC();
//    ret = GTP_GPIO_REQUEST(gpio_irq_port, "GTP_INT_IRQ");
    ret = GTP_GPIO_REQUEST(gpio_irq_port, "GTP_INT_IRQ");
    if (ret < 0) 
    {
        GTP_ERROR("Failed to request GPIO:%d, ERRNO:%d", (s32)gpio_irq_port, ret);
        ret = -ENODEV;
    }
    else
    {
        GTP_GPIO_AS_INT(gpio_irq_port);  
        ts->client->irq = gpio_to_irq(gpio_irq_port);
//        ts->client->irq = GTP_INT_IRQ;
    }

    ret = GTP_GPIO_REQUEST(gpio_reset_port, "GTP_RST_PORT");
    if (ret < 0) 
    {
        GTP_ERROR("Failed to request GPIO:%d, ERRNO:%d", (s32)gpio_reset_port,ret);
        ret = -ENODEV;
    }

    GTP_GPIO_AS_INPUT(gpio_reset_port);

    gtp_reset_guitar(ts->client, 20);
    
    if(ret < 0)
    {
        GTP_GPIO_FREE(gpio_reset_port);
        GTP_GPIO_FREE(gpio_irq_port);
    }
...
static s8 gtp_request_irq(struct goodix_ts_data *ts)
{
    ...
    if (ret)
    {
        ...
        GTP_GPIO_AS_INPUT(gpio_irq_port);
        GTP_GPIO_FREE(gpio_irq_port);
...
#endif
    ret = gup_clk_calibration(gpio_irq_port);
    gtp_esd_recovery(ts->client);
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	of_property_read_u32(client->dev.of_node, "interrupt-gpio", &ts->int_gpio);
	of_property_read_u32(client->dev.of_node, "reset-gpio", &ts->reset_gpio);
    ...
    ts->int_trigger_type = GTP_INT_TRIGGER;
printk("goodix_int = %d; goodix_reset_gpio = %d; goodix_int_gpio = %d ;\n", client->irq, ts->reset_gpio, ts->int_gpio);
printk("gootix gt911 touchscreen configuration: x_max = %d, y_max = %d, trigger_type = 0x%08x\n", ts->abs_x_max, ts->abs_y_max, ts->int_trigger_type);
